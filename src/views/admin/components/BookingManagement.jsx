...
import { Search, Filter, CheckCircle, Calendar, DollarSign, RefreshCw } from 'lucide-react'
...
export function BookingManagement({ onNavigate }) {
  // REGLA DE NEGOCIO: Los estados 'CONFIRMADA' y 'RECHAZADA' son estados finales.
  const FINAL_STATUSES = ['RECHAZADA', 'CONFIRMADA']
    ...
    try {
      ...
      const normalized = (bookingsData.bookings || []).map(item => ({
        ...
        arrendatario:
          ...
          (item.propiedad?.id_arrendatario
            ? `Arrendatario #${item.propiedad.id_arrendatario}`
            : 'Sin arrendatario'),
      }))
  ...
  const filteredBookings = useMemo(
    () =>
      bookings.filter(booking => {
        ...
        const matchesStatus =
          filterStatus === 'all' ||
          booking.estado.toLowerCase().includes(filterStatus.toLowerCase())
  ...
  const getStatusBadgeColor = status => {
    ...
    if (normStatus === 'CONFIRMADA') return 'bg-[#6B8E23] text-white'
    if (normStatus === 'PENDIENTE') return 'bg-yellow-100 text-yellow-700'
    if (normStatus === 'RECHAZADA') return 'bg-red-100 text-red-700'
    if (normStatus === 'FINALIZADA') return 'bg-blue-100 text-blue-700'
  ...
  const getStatusLabel = status => {
    const normStatus = normalizeStatus(status)
    return normStatus.charAt(0) + normStatus.slice(1).toLowerCase()
  }
  ...
  const getAccionesPorEstado = estado => {
    ...
    return [
      ...
      { id: 'rechazar', label: 'Rechazar', variant: 'red' },
    ]
  }
  ...
  const updateBookingStatus = async (booking, nextStatus) => {
    ...
    if (!updatedRows || updatedRows.length === 0) {
      showActionMessage(
        'error',
        'No se actualizó la reserva en la base de datos. Revisa permisos RLS de UPDATE en tabla reserva.'
      )
      return false
    }

    setBookings(prev =>
      prev.map(item => (item.key === booking.key ? { ...item, estado: nextStatus } : item))
    )
  ...
  const actionHandlers = {
    ...
    cancelar: rechazarReserva, // 'cancelar' ahora usa la lógica de 'rechazar'
  ...
  const getActionModalConfig = () => {
    ...
    if (confirmAction.actionId === 'eliminar') {
      return {
        ...
        description: (
          <p>
            ¿Seguro quieres eliminar la reserva <strong>{bookingLabel}</strong>? Esta acción no se
            puede deshacer.
          </p>
        ),
  ...
  return (
    <div className="p-6 lg:p-8 space-y-6">
      <div>
        <h1 className="font-['Poppins'] font-semibold text-[#5F5F5F] mb-2">Gestión de Reservas</h1>
        <p className="text-[#5F5F5F]/70">Administra la tabla reserva con datos reales</p>
      </div>
          ...
          <CardContent className="p-4">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-['Poppins'] font-semibold text-[#5F5F5F]">
                  {bookings.filter(b => normalizeStatus(b.estado) === 'CONFIRMADA').length}
                </p>
              </div>
          ...
          <CardContent className="p-4">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-['Poppins'] font-semibold text-[#5F5F5F]">
                  {bookings.filter(b => normalizeStatus(b.estado) === 'PENDIENTE').length}
                </p>
              </div>
                ...
                {filteredBookings.map(booking => (
                  <TableRow key={booking.key} className="hover:bg-[#F2E8CF]/20">
                    ...
                    <TableCell>
                      ...
                      <p className="text-sm text-[#5F5F5F]/60">
                        Arrendatario: {booking.arrendatario}
                      </p>
                    </TableCell>
